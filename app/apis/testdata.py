...
api = Namespace(
    "test-data",
    description="Test Data operations, simulating real data gathered from IOT device via MQTT protocol",
)
...
test_data = api.model(
    ...
    {
        "id": fields.Integer(
            readOnly=True, description="The unique identifier of a test data"
        ),
        "value": fields.String(required=True, description="The value of the test data"),
        "register_time": fields.DateTime(
            dt_format="iso8601", description="The time the test data was registered"
        ),
    },
)
...
class TestDataResource(Resource):
    ...
    def get(self, start_date, end_date):
        try:
            ...
            date_series = select(
                cast(
                    func.generate_series(start_date, end_date, timedelta(days=1)), Date
                ).label("day")
            ).subquery()
    ...
    def get(self, date):
        try:
            ...
            query = (
                ...
                .filter(func.date(TestData.register_time) == date)
                .group_by(func.date_part("hour", TestData.register_time))
            )
